...
import { FaUserTie, FaEnvelope, FaPhone, FaSearch, FaEye, FaEdit, FaTrash, FaPlus, FaCalendar, FaMapMarkerAlt } from 'react-icons/fa';
...
const ManageEmployees = () => {
  ...
  const handleStatusToggle = async (employeeId, currentStatus) => {
    try {
      await adminAPI.updateEmployee(employeeId, { isActive: !currentStatus });
      loadEmployees();
    } catch (error) {
                ...
                <div className="flex items-center">
                  ...
                  <div className="ml-5 w-0 flex-1">
                    <dl>
                      ...
                      <dd className="text-lg font-medium text-gray-900">
                        {employees.filter(e => e.position === 'teacher').length}
                      </dd>
                    </dl>
                  </div>
                </div>
            ...
            <div className="bg-white overflow-hidden shadow rounded-lg">
              <div className="px-4 py-5 sm:p-6">
                <div className="flex items-center">
                  ...
                  <div className="ml-5 w-0 flex-1">
                    <dl>
                      <dt className="text-sm font-medium text-gray-500 truncate">Avg Tenure</dt>
                      <dd className="text-lg font-medium text-gray-900">
                        {(
                          employees.reduce((acc, emp) => {
                            const hireDate = new Date(emp.hireDate);
                            const today = new Date();
                            const years = (today - hireDate) / (365 * 24 * 60 * 60 * 1000);
                            return acc + years;
                          }, 0) / employees.length
                        ).toFixed(1)} years
                      </dd>
                  ...
                  {filteredEmployees.length === 0 ? (
                    ...
                    filteredEmployees.map((employee) => (
                      <tr key={employee._id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getPositionColor(employee.position)}`}>
                            {employee.position}
                          </span>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm text-gray-900">
                            {employee.classroom?.name || 'Not assigned'}
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <p className="mt-1 text-sm text-gray-900 capitalize">{selectedEmployee.position}</p>
                      </div>
                      <div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <p className="mt-1 text-sm text-gray-900">
                          {selectedEmployee.classroom?.name || 'Not assigned'}
                        </p>
                      </div>
                    </div>
                    ...
                    {selectedEmployee.qualifications && (
                      <div>
                        ...
                        <p className="mt-1 text-sm text-gray-900">{selectedEmployee.qualifications}</p>
                      </div>
                    )}
                    ...
                    <div>
                      ...
                      {selectedEmployee.emergencyContact ? (
                        <div className="mt-1 grid grid-cols-3 gap-4 text-sm">
                          ...
                          <div>
                            <span className="font-medium">Relationship:</span> {selectedEmployee.emergencyContact.relationship}
                          </div>
                        </div>
                      ) : (
